  ...
  if (!showFields.length) {
    return (
      <Flex justify="center" align="center" style={{ height: '100%', color: '#9e9e9e' }}>
        Please configure the display field in the view configuration first
      </Flex>
    );
  }
                ...
                if (row.isSub && row.key.includes('more') && subFieldcontrol.showControls.length <= index + 1) {
                  return (
                    <Flex justify="flex-end">
                      <div className="view-more" onClick={() => handleLoadMoreRelationRows(row.key.replace('more-', ''))}>more...</div>
                    </Flex>
                  );
                }
