...
function getError(mapViewConfig, env) {
  if (!mapViewConfig.positionId) {
    return "Please configure the corresponding field of \"Location\" in the view configuration first";
  } else if (!env.amap_key) {
    return `Please configure the Autonavi map key in JSON format in the environment parameters ${JSON.stringify({
      amap_key: "xxxxxxxxxxx",
    })}<br/>(The developing plugins are configured in the view configuration, and the published plugins are configured in the \"Plugins\".)`;
  } else {
    return;
  }
}
